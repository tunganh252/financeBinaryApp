...
import { actionRefreshState } from '../../stores/actions/user'
import { otpRequiredMsgError } from './constant'

export const LoginScreen = ({ navigation }) => {
  // ====== Stores ====== //
  const dispatcher = useDispatch()
  const { state: userReducer, post: postLoginUser } = useUserLoginSetting()
  const { execute: postLoginUserAsync, status: postLoginUserStatus } = useAsync(postLoginUser)
  ...
  const _handleLoginWithOTP = () => {
    const { email, password, otpCode } = valInput
    if (!email || !password) {
      createAlert('Warning', 'Please complete all information!')
      return
    }
    if (!otpCode) {
      createAlert('Warning', 'OTP required.')
      return
    }
    postLoginUserAsync({
      username: email.trim(),
  ...
  const createAlert = (title, msg) => Alert.alert(title, msg, [{ text: 'OK', style: 'cancel' }])
  ...
  useEffect(() => {
    if (!!userReducer.error && userReducer.type === '@user/login-error') {
      if (userReducer.error === otpRequiredMsgError) {
  ...
  return (
    <>
              ...
              <View style={styles.viewForm}>
                <View>
                  ...
                  <View style={{ marginTop: 40, position: 'relative' }}>
                    ...
                    >
                      <IconEyePass width={25} height={25} color={COLORS.white} active={valInput.isHidePassword} />
                    </TouchableOpacity>
                  </View>
                </View>
                ...
                <View style={styles.viewJuridical}>
                  <Text style={styles.textJuridical}>Continue the registration if you agree to</Text>
                  <OpenURLButton url={'https://thisoption.com/terms'}>Term of Use.</OpenURLButton>
                </View>
              </View>
            </TouchableWithoutFeedback>
          </KeyboardAvoidingView>
      ...
      <Modal
        animationType="slide"
        transparent={true}
        visible={isShowPopupOTP}
        onRequestClose={() => {
          setIsShowPopupOTP(!isShowPopupOTP)
        }}
      >
        <View style={styles.viewCardModal}>
          ...
          <View style={[styles.viewContentModal, styles.shadow]}>
            ...
            <View style={styles.viewBtnModal}>
              ...
              <View
                style={{
                  ...styles.viewBtnLoginModal,
                  flex: 1,
                  marginHorizontal: 5,
                  marginTop: 35,
                }}
              >
                <TouchableOpacity style={styles.touch__LoginModal} onPress={_handleLoginWithOTP}>
                  <Text style={{ color: COLORS.white, marginHorizontal: 30 }}>Login</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
