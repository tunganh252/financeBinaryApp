  ...
  const post = async (data) => {
    ...
    return await UserServices.POST_LOGIN_USER(data)
      .then(async (response) => {
        ...
        if (!success && message === 'otp required.') {
          let msg = response.data?.message || 'Invalid credentials.'
        ...
        if (!!success && !!data) {
          dispatcher(actionLoginUser(data))
        }
      })
      .catch((err) => {
        let msg = err?.error?.data?.message || 'Invalid credentials.'
...
export const useGetProfile = () => {
  ...
  const dispatcher = useDispatch()
  const get = async () => {
    return await UserServices.GET_PROFILE()
      .then((response) => {
        dispatcher(actionGetProfile(response.data.data))
      })
      .catch((err) => {
        dispatcher(actionLogout())
      })
