...
const Wallet_Partner = ({ navigation, isShowEyes, setLoadingPage }) => {
   ...
   * Stores
   */
  const { state: walletPartnerReducer, get: getWalletPartner } = useWalletPartnerGetAllSetting()
  const { execute: getWalletPartnerAsync, status: getWalletPartnerStatus } = useAsync(getWalletPartner)
  ...
  useEffect(() => {
    if (getWalletPartnerStatus === 'loading' || getAllListCoinsStatus === 'loading') setLoadingPage(true)
    ...
    <ScrollView>
      ...
      <View>
        <Condition
          If={{
            condition:
              !!walletPartnerReducer.data &&
              !!listCoinsReducer &&
              walletPartnerReducer.data.length > 0 &&
              ...
              <Loop
                ...
                onRender={(item, index) => {
                  ...
                  return (
                    <TouchableOpacity
                      key={item.name}
                      onPress={() => navigation.navigate('wallet', { ...item, route: 'partnerDetail' })}
                    >
                      <View>
