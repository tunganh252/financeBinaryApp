...
import { convertNumToMoney, wait_macroTask } from 'constant/helpers/function'
...
import Wallet_Fiat from 'components/organisms/Wallet_Fiat'
      ...
      <View style={{ ...styles.viewSectionHeader, ...styles.shadow }}>
        <View style={styles.viewBgHeader}>
          ...
          <View style={styles.viewHeaderBar__eye}>
            <TouchableOpacity style={styles.touchAble__icon_eye} onPress={_handleEyes}>
              {/* <View style={{ flex: 1 }}> */}
              <IconEyePass width={30} height={30} color={'#ffffff'} active={isShowEyes} />
              {/* </View> */}
            </TouchableOpacity>
          </View>
            ...
            <View>
              ...
              <View>
                ...
                <Text style={styles.textConver__coint}>
                  &asymp; {isShowEyes ? '****' : convertNumToMoney(10234, '.', ' USD', false)}
                </Text>
              </View>
            </View>
          </View>
      ...
      <>
        <View>
          ...
          <View style={styles.viewContentData}>
            {tab.key === 'exchange' && (
              <Wallet_Exchange navigation={navigation} isShowEyes={isShowEyes} setLoadingPage={setLoadingPage} />
            )}
            {tab.key === 'investment' && (
              <Wallet_Investment navigation={navigation} isShowEyes={isShowEyes} setLoadingPage={setLoadingPage} />
            )}
            {tab.key === 'partner' && (
              <Wallet_Partner navigation={navigation} isShowEyes={isShowEyes} setLoadingPage={setLoadingPage} />
            )}
            {/* {tab.key === 'fiat' && (
              <Wallet_Fiat navigation={navigation} isShowEyes={isShowEyes} setLoadingPage={setLoadingPage} />
            )} */}
          </View>
        </View>
      </>
